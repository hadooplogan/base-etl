-- 同电话名单
create table rn_baseinfo_same_tel_20170424 as 
select tel
  from (select tel, count(distinct pripid) tel_count
          from (select *
                  from enterprisebaseinfocollect_merge_20170417
 ...
 where length(tel) > 5;
create table rn_baseinfo_same_tel_rela_20170424 as 
select me.pripid,
       ai.tel,
       concat_ws('#', me.pripid, '2') as spid,
       ai.tel as epid
  from (select tel from (select tel, count(distinct pripid) tel_count
          from (select *
                  from enterprisebaseinfocollect_merge_20170417
                 where tel regexp '^(13|15|18|17)[0-9]{9}$'
                    or tel regexp '^[(|（]?(13|15|18|17)[0-9]{9}[-|(|)|）|\\|*| |,|，|/]{1,6}+(13|15|18|17)[0-9]{9}'
         ...
         where entstatus = '1'
           ...
           and tel <> ''
         group by tel
        having count(distinct pripid) > 1 and count(distinct pripid) < 5) where length(tel)>5) ai
