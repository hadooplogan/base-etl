 ...
 */
public class topLessCredit {



    public static Dataset getTopLessCredit(SparkSession spark,String datadate){

                 tmpForJoin1(spark,datadate);
                 tmpForJoin1a(spark,datadate);
                 tmpForJoin2(spark,datadate);
                 tmpForGroup(spark,datadate);
        return  LessCreditTimes(spark, datadate);
   ...
   private static Dataset tmpForJoin1(SparkSession spark ,String datadate) {
        String hql = "select b.zspid ,b.pripid from \n" +
                "(select zspid ,pripid from e_pri_person_full_20170927 where pripid is not  null and pripid <> '') b \n" +
                "join \n" +
    ...
    private static Dataset tmpForJoin1a(SparkSession spark,String datadate){
      ...
      private static Dataset tmpForGroup(SparkSession spark,String datadate){
            ...
            String hql = "select b.pripid  from tmpforjoin2 a join tmpforjoin1a b \n" +
                    "on a.zspid = b.zspid ";
            return DataFrameUtil.getDataFrame(spark,hql.replaceAll("datadate",datadate),"tmpforgroup");
        }

   private static Dataset LessCreditTimes(SparkSession spark,String datadate) {

           String hql ="select pripid, count(1) as lesscredittmies  from tmpforgroup group by pripid ";

        return DataFrameUtil.getDataFrame(spark,hql.replaceAll("datadate",datadate),"toplesscreditkpi");
